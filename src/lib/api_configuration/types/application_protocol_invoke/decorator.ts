...
import * as application_protocol_invoke from "./read_api";
import * as application_protocol_invoke_resolver from "./reference_resolver";
...
export type Naked_application_protocol_invoke = {
	'command': string;
	_typescript_dummy_application_protocol_invoke?:boolean;
};
export function imp_decorate_application_protocol_invoke(api, $:Naked_application_protocol_invoke, containing_node, meta_table, onError, input_parameters, lazy = false) { 
	var data = new api.Capplication_protocol_invoke();
	...
	data.properties.command = $["command"];
	return data;
}
export function int_decorate(api, $:Naked_application_protocol_invoke, input_parameters?, onError?:(err?:string) => void, lazy = false) {
	...
	var node = imp_decorate_application_protocol_invoke(api, $, null, meta_table, onError, input_parameters, lazy);
	if (!lazy) {
		//reference resolving
		var ri;
...
export function decorate($:Naked_application_protocol_invoke, input_parameters:{}, onError?:(err?:string) => void):read_api.Capplication_protocol_invoke {
	return int_decorate(read_api, $, input_parameters, onError, false);
}
export function lazy_decorate($:Naked_application_protocol_invoke, input_parameters:{}, onError?:(err?:string) => void):read_api.Capplication_protocol_invoke {
